...
Public Class importPrices
    ...
    Private Sub doBrowse()
        'msgbox("*******doBrowse")
        fileDialogPrices.InitialDirectory = System.AppDomain.CurrentDomain.BaseDirectory()
    ...
    Private Sub readExcelFileHelper()
        Try
            ...
            Dim uniqueFileName As String = System.AppDomain.CurrentDomain.BaseDirectory() & "//excelFileImport" & Now.TimeOfDay.ToString.Replace(":", "").Replace(".", "") & ".log"
    ...
    Private Sub importPrices_Load(sender As Object, e As EventArgs) Handles Me.Load
        'File.Create(System.AppDomain.CurrentDomain.BaseDirectory()  & "//debugConsoleOut.out")
        If Not File.Exists(System.AppDomain.CurrentDomain.BaseDirectory() & "//debugConsoleOut.out") Then
            Dim fileStre As FileStream = File.Create(System.AppDomain.CurrentDomain.BaseDirectory() & "//debugConsoleOut.out")
            fileStre.Close()
        End If

        Try
            objWriter = New System.IO.StreamWriter(System.AppDomain.CurrentDomain.BaseDirectory() & "\\debugConsoleOut.out", True)
            'msgbox("*******file visited")
        Catch ex As Exception
            'msgbox(ex.Message)
        End Try
